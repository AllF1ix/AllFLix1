...
const SITE_URL = "https://allf1ix.github.io";
const SITE_NAME = "AllFlix";
const DEFAULT_DESCRIPTION = "Panoorin ang pinakabagong official trailers ng mga movies, TV shows, K-Drama, at anime mula sa buong mundo. Laging updated, libre, walang account na kailangan.";
...
function parseDetailPath(pathname) {
    const match = pathname.match(/^\/(movie|tv)\/[a-z0-9-]*?-(\d+)\/?$/i);
...
function updateSEO(data, type, path) {
    ...
    if (data) {
        ...
        image = data.backdrop_path ? (BACKDROP_URL + data.backdrop_path) : (data.poster_path ? (IMG_URL + data.poster_path) : (SITE_URL + "/og-default.jpg"));
        ogType = "video.other";
    } else {
...
function renderGrid(items, limit = MAX_CATALOG_ITEMS, showNumbers = false) {
    ...
    const htmlParts = limitedItems.map((item, index) => {
        ...
        return `
            <div class="card" onclick="openDetail('${item.id}', '${type}')" data-orig-title="${safeOriginalTitle}" data-orig-lang="${item.original_language || ""}">
